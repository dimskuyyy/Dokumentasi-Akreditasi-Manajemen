<div class="pull-left">
    <button type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-plus"></i> Tambah Penelitian</button>
</div><br><br>
<table id="table-penelitian" class="table table-bordered table-hover table-striped" style="width:100%">
    <thead>
        <tr>
...
<script>
    $('#table-penelitian').on('click', 'tbody tr td .btn-edit', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
    ...
    $('#table-penelitian').on('click', 'tbody tr td .btn-delete', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
        var id = btn.attr('data-id');
            ...
            if (confirm('Yakin hapus post ?')) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status) {
                            successMsg(res.msg);
                            $('#table-penelitian').DataTable().ajax.reload();
                        } else {
                            errorMsg(res.msg);
                        }
                        resetLoadingBtn(btn, htm);
    ...
    var oTable = $('#table-penelitian').dataTable({
        'bFilter': false,
        'autoWidth': true,
    ...
    $('#table-penelitian').on('click', '.btn-look', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/penelitian/info',
                type: 'post',
                data: {
                    id: id
                },
