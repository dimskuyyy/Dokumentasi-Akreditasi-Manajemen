<div class="pull-left">
    <button type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-plus"></i> Tambah Surat Tugas</button>
</div><br><br>
<table id="table-surat-tugas" class="table table-bordered table-hover table-striped" style="width:100%">
    <thead>
        <tr>
            <th data-priority='1'>Nama Surat Tugas</th>
            <th data-priority='1'>Nomor Surat Tugas</th>
...
<script>
    $('#table-surat-tugas').on('click', 'tbody tr td .btn-edit', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
    ...
    $('#table-surat-tugas').on('click', 'tbody tr td .btn-delete', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
        var id = btn.attr('data-id');
        if (id) {
            if (confirm('Yakin hapus post ?')) {
                setLoadingBtn(btn);
                $.ajax({
                    url: base_url + '/surat-tugas/delete',
                    type: 'post',
                    data: {
                        id: id
                    },
                    success: function(res) {
                        if (res.status) {
                            successMsg(res.msg);
                            $('#table-surat-tugas').DataTable().ajax.reload();
                        } else {
                            errorMsg(res.msg);
                        }
                        resetLoadingBtn(btn, htm);
    ...
    var oTable = $('#table-surat-tugas').dataTable({
        'bFilter': false,
        'autoWidth': true,
    ...
    $('#table-surat-tugas').on('click', '.btn-look', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/surat-tugas/info',
                type: 'post',
                data: {
                    id: id
                },
