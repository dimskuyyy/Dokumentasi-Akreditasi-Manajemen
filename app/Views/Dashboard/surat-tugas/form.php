...
<div class="box box-widget">
    <div class="box-body">
        <div class="pull-left">
            <button class="btn btn-sm btn-flat btn-primary btn-list"><i class="fa fa-list"></i> List Surat Tugas</button>
        </div><br><br>
        <?php echo form_open('#', ['class' => 'form-post']);
        if (isset($data)) {
            echo form_hidden('id', $data['dokumen_id']);
        }
        ?>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label for='nama'>Nama Surat Tugas</label><br>
                        <input type="text" name="nama" class="form-control" value="<?= isset($data) ? $data['dokumen_nama'] : '' ?>" required>
                    </div>
                    <div class="form-group">
                        <label for='nomor'>Nomor Surat Tugas</label><br>
                        <input type="text" name="nomor" class="form-control" value="<?= isset($data) ? $data['dokumen_nomor'] : '' ?>" required>
                    </div>
                    <div class="form-group">
                        <label for='media'>Media</label><br>
                        <div class="choose-media">
                            <a href="#" class="btn btn-sm btn-flat btn-primary btn-media" data-backdrop="static"><i class="fa fa-plus"></i> Unggah Dokumen Surat Tugas<span id="text-optional"></span></a>
                            <button type="button" class="btn btn-default btn-flat btn-sm btn-reset-media"><i class="fa fa-recycle" aria-hidden="true"></i></button>
                        </div>
                        <input type="hidden" name="media" id="media-id" value="<?= isset($data) ? $data['dokumen_media_id'] : '' ?>">
...
<script>
    $(document).ready(function() {
        ...
        $('.btn-media').click(function(e) {
            ...
            setLoadingBtn(btn);
            $.ajax({
                url: base_url + '/surat-tugas/media',
                data: {
                    key: 'cover'
                },
                type: 'post',
