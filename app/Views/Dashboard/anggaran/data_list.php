<div class="pull-left">
    <button type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-plus"></i> Tambah Dokumen Anggaran</button>
</div><br><br>
<table id="table-anggaran" class="table table-bordered table-hover table-striped" style="width:100%">
    <thead>
        <tr>
            <th data-priority='1'>Tahun Anggaran</th>
...
<script>
    $('#table-anggaran').on('click', 'tbody tr td .btn-edit', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
    ...
    $('#table-anggaran').on('click', 'tbody tr td .btn-delete', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
        var id = btn.attr('data-id');
            ...
            if (confirm('Yakin hapus post ?')) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status) {
                            successMsg(res.msg);
                            $('#table-anggaran').DataTable().ajax.reload();
                        } else {
                            errorMsg(res.msg);
                        }
                        resetLoadingBtn(btn, htm);
    ...
    var oTable = $('#table-anggaran').dataTable({
        'bFilter': false,
        'autoWidth': true,
    ...
    $('#table-anggaran').on('click', '.btn-look', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/anggaran/info',
                type: 'post',
                data: {
                    id: id
                },
