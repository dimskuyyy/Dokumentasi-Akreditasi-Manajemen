...
<div class="box box-widget">
    <div class="box-body">
        <div class="pull-left">
            <button class="btn btn-sm btn-flat btn-primary btn-list"><i class="fa fa-list"></i> List HaKi</button>
        </div><br><br>
        <?php echo form_open('#', ['class' => 'form-post']);
        if (isset($data)) {
            echo form_hidden('id', $data['haki_id']);
        }
        ?>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label for='nama'>Nama HaKi</label><br>
                        <input type="text" name="nama" class="form-control" value="<?= isset($data) ? $data['haki_nama'] : '' ?>" required>
                    </div>
                    <div class="form-group">
                        <label for='klasifikasi'>Klasifikasi HaKi</label><br>
                        <input type="text" name="klasifikasi" class="form-control" value="<?= isset($data) ? $data['haki_klasifikasi'] : '' ?>" required>
                    </div>
                    <div class="form-group">
                        <label for='nomor'>Nomor HaKi</label><br>
                        <input type="text" name="nomor" class="form-control" value="<?= isset($data) ? $data['haki_nomor'] : '' ?>" required>
                    </div>
                    <div class="form-group">
                        <label for='media'>Media</label><br>
                        <div class="choose-media">
                            <a href="#" class="btn btn-sm btn-flat btn-primary btn-media" data-backdrop="static"><i class="fa fa-plus"></i> Unggah Dokumen HaKi<span id="text-optional"></span></a>
                            <button type="button" class="btn btn-default btn-flat btn-sm btn-reset-media"><i class="fa fa-recycle" aria-hidden="true"></i></button>
                        </div>
                        <input type="hidden" name="media" id="media-id" value="<?= isset($data) ? $data['haki_media_id'] : '' ?>">
                        <div class="source-media">
                            ...
                            <?php } ?>
                            <p class="note-media <?= isset($media) ? 'active' : '' ?>">Tidak Ada Dokumen</p>
                        </div>
                    </div>
                </div>
            </div>
