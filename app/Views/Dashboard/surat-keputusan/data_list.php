<div class="pull-left">
    <button type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-plus"></i> Tambah SK</button>
</div><br><br>
<table id="table-surat-keputusan" class="table table-bordered table-hover table-striped" style="width:100%">
    <thead>
        <tr>
            <th data-priority='1'>Nama SK</th>
            <th data-priority='1'>Nomor SK</th>
...
<script>
    $('#table-surat-keputusan').on('click', 'tbody tr td .btn-edit', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
        ...
        if (id) {
            setLoadingBtn(btn);
            $.ajax({
                url: base_url + '/surat-keputusan/form',
                type: 'post',
                data: {
                    id: id
                },
    ...
    $('#table-surat-keputusan').on('click', 'tbody tr td .btn-delete', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
        var id = btn.attr('data-id');
        if (id) {
            if (confirm('Yakin hapus post ?')) {
                setLoadingBtn(btn);
                $.ajax({
                    url: base_url + '/surat-keputusan/delete',
                    type: 'post',
                    data: {
                        id: id
                    },
                    success: function(res) {
                        if (res.status) {
                            successMsg(res.msg);
                            $('#table-surat-keputusan').DataTable().ajax.reload();
                        } else {
                            errorMsg(res.msg);
                        }
                        resetLoadingBtn(btn, htm);
    ...
    var oTable = $('#table-surat-keputusan').dataTable({
        'bFilter': false,
        'autoWidth': true,
        ...
        'ajax': function(data, callback, setting) {
            $.ajax({
                url: base_url + '/surat-keputusan/list',
                type: 'post',
                data: {
                    datatables: data
                },
    ...
    $('#table-surat-keputusan').on('click', '.btn-look', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/surat-keputusan/info',
                type: 'post',
                data: {
                    id: id
                },
