<div class="pull-left">
    <button type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-plus"></i> Tambah IPK Mahasiswa</button>
</div><br><br>
<table id="table-ipk-mahasiswa" class="table table-bordered table-hover table-striped" style="width:100%">
    <thead>
        <tr>
            <th data-priority='1'>Tahun</th>
            <th data-priority='1'>Rata Rata IPK</th>
...
<script>
    $('#table-ipk-mahasiswa').on('click', 'tbody tr td .btn-edit', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
        ...
        if (id) {
            setLoadingBtn(btn);
            $.ajax({
                url: base_url + '/ipk-mahasiswa/form',
                type: 'post',
                data: {
                    id: id
                },
    ...
    $('#table-ipk-mahasiswa').on('click', 'tbody tr td .btn-delete', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
        var id = btn.attr('data-id');
        if (id) {
            if (confirm('Yakin hapus post ?')) {
                setLoadingBtn(btn);
                $.ajax({
                    url: base_url + '/ipk-mahasiswa/delete',
                    type: 'post',
                    data: {
                        id: id
                    },
                    success: function(res) {
                        if (res.status) {
                            successMsg(res.msg);
                            $('#table-ipk-mahasiswa').DataTable().ajax.reload();
                        } else {
                            errorMsg(res.msg);
                        }
                        resetLoadingBtn(btn, htm);
    ...
    var oTable = $('#table-ipk-mahasiswa').dataTable({
        'bFilter': false,
        'autoWidth': true,
        ...
        'ajax': function(data, callback, setting) {
            $.ajax({
                url: base_url + '/ipk-mahasiswa/list',
                type: 'post',
                data: {
                    datatables: data
                },
    ...
    $('#table-ipk-mahasiswa').on('click', '.btn-look', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/ipk-mahasiswa/info',
                type: 'post',
                data: {
                    id: id
                },
