<div class="pull-left">
    <?php
    if (is_null($data)) { ?>
        <button type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-plus"></i> Tambah Dokumen Panduan Akademik</button>
    <?php } else { ?>
        <button type="button" class="btn btn-sm btn-flat btn-primary btn-create" disabled><i class="fa fa-plus"></i> Tambah Dokumen Panduan Akademik</button>
    <?php } ?>
</div><br><br>
<table id="table-panduan-akademik" class="table table-bordered table-hover table-striped" style="width:100%">
    <thead>
        <tr>
            <th data-priority='1'>Dokumen</th>
            <th data-priority='1'>Oleh</th>
...
<script>
    $('#table-panduan-akademik').on('click', 'tbody tr td .btn-edit', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
        ...
        if (id) {
            setLoadingBtn(btn);
            $.ajax({
                url: base_url + '/panduan-akademik/form',
                type: 'post',
                data: {
                    id: id
                },
    ...
    $('#table-panduan-akademik').on('click', 'tbody tr td .btn-delete', function(e) {
        e.preventDefault();
        var btn = $(this);
        var htm = btn.html();
        var id = btn.attr('data-id');
        if (id) {
            if (confirm('Yakin hapus post ?')) {
                setLoadingBtn(btn);
                $.ajax({
                    url: base_url + '/panduan-akademik/delete',
                    type: 'post',
                    data: {
                        id: id
                    },
                    success: function(res) {
                        if (res.status) {
                            successMsg(res.msg);
                            $('#table-panduan-akademik').DataTable().ajax.reload();
                        } else {
                            errorMsg(res.msg);
                        }
                        resetLoadingBtn(btn, htm);
    ...
    var oTable = $('#table-panduan-akademik').dataTable({
        'bFilter': false,
        'autoWidth': true,
        ...
        'columns': [{
                ...
                'render': function(data, type, row) {
                    return ' <button type=\'submit\' class=\'btn btn-sm btn-warning btn-flat\'> Panduan Akademik </button> ';
                }
            },
            {
                data: 'oleh',
        ...
        'ajax': function(data, callback, setting) {
            $.ajax({
                url: base_url + '/panduan-akademik/list',
                type: 'post',
                data: {
                    datatables: data
                },
    ...
    $('#table-panduan-akademik').on('click', '.btn-look', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/panduan-akademik/info',
                type: 'post',
                data: {
                    id: id
                },
