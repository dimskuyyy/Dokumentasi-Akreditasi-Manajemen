<div class="pull-left" style="display: flex;align-items:center; gap:1rem; margin-bottom: 1rem;">
    <a href="<?=base_url('wbpanel/check-tendik')?>" type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-arrow-left"></i></a> <h4 style="display: inline-block;">IPK Mahasiswa</h4>
</div><br><br>
...
<script>
    ...
    var oTable = $('#table-ipk-mahasiswa').dataTable({
        ...
        'ajax': function(data, callback, setting) {
            $.ajax({
                url: base_url + '/check-tendik/ipk-mahasiswa/list',
                type: 'post',
                data: {
                    datatables: data,
                    id:id
    ...
    $('#table-ipk-mahasiswa').on('click', '.btn-detail-media', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/check-tendik/ipk-mahasiswa/info',
                type: 'post',
                data: {
                    id: id
                },
