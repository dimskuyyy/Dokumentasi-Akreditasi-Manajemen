<div class="pull-left" style="display: flex;align-items:center; gap:1rem; margin-bottom: 1rem;">
    <a href="<?=base_url('wbpanel/check-tendik')?>" type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-arrow-left"></i></a> <h4 style="display: inline-block;">Sertifikat Kompetensi/Profesi Tenaga Kependidikan</h4>
</div><br><br>
<table id="table-sertifikat" class="table table-bordered table-hover table-striped" style="width:100%">
    <thead>
        <tr>
            ...
            <th data-priority='1'>Nomor Sertifikat</th>
...
<script>
    ...
    var oTable = $('#table-sertifikat').dataTable({
        ...
        'ajax': function(data, callback, setting) {
            $.ajax({
                url: base_url + '/check-tendik/sertifikat/list',
                type: 'post',
                data: {
                    datatables: data,
                    id:id
    ...
    $('#table-sertifikat').on('click', '.btn-detail-media', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/check-tendik/sertifikat/info',
                type: 'post',
                data: {
                    id: id
                },
