<div class="pull-left" style="display: flex;align-items:center; gap:1rem; margin-bottom: 1rem;">
    <a href="<?=base_url('wbpanel/check-dosen')?>" type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-arrow-left"></i></a> <h4 style="display: inline-block;">SK Pengajaran</h4>
</div><br><br>
<table id="table-pengajaran-dosen" class="table table-bordered table-hover table-striped" style="width:100%">
    <thead>
        <tr>
            <th data-priority='1'>Pengunggah</th>
            <th data-priority='1'>Nama SK Pengajaran</th>
...
<script>
    let id = <?=$uid?>;
    var oTable = $('#table-pengajaran-dosen').dataTable({
        'bFilter': false,
        'autoWidth': true,
        ...
        'ajax': function(data, callback, setting) {
            $.ajax({
                url: base_url + '/check-dosen/pengajaran-dosen/list',
                type: 'post',
                data: {
                    datatables: data,
                    id:id
    ...
    $('#table-pengajaran-dosen').on('click', '.btn-detail-media', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/check-dosen/pengajaran-dosen/info',
                type: 'post',
                data: {
                    id: id
                },
