<div class="pull-left" style="display: flex;align-items:center; gap:1rem; margin-bottom: 1rem;">
    <a href="<?=base_url('wbpanel/check-dosen')?>" type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-arrow-left"></i></a> <h4 style="display: inline-block;">Kegiatan Dalam Kampus</h4>
</div><br><br>
<table id="table-kegiatan-dalam" class="table table-bordered table-hover table-striped" style="width:100%">
    <thead>
        <tr>
...
<script>
    let id = <?=$uid?>;
    var oTable = $('#table-kegiatan-dalam').dataTable({
        'bFilter': false,
        'autoWidth': true,
        ...
        'ajax': function(data, callback, setting) {
            $.ajax({
                url: base_url + '/check-dosen/kegiatan/list/dalam-kampus',
                type: 'post',
                data: {
                    datatables: data,
                    id:id
    ...
    $('#table-kegiatan-dalam').on('click', '.btn-detail-media', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/check-dosen/kegiatan/info/dalam-kampus',
                type: 'post',
                data: {
                    id: id
                },
