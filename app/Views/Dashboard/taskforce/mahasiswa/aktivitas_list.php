<div class="pull-left" style="display: flex;align-items:center; gap:1rem; margin-bottom: 1rem;">
    <a href="<?=base_url('wbpanel/check-mahasiswa')?>" type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-arrow-left"></i></a> <h4 style="display: inline-block;">Aktivitas Mahasiswa</h4>
</div><br><br>
...
<script>
    ...
    var oTable = $('#table-aktivitas').dataTable({
        ...
        'ajax': function(data, callback, setting) {
            $.ajax({
                url: base_url + '/check-mahasiswa/aktivitas/list',
                type: 'post',
                data: {
                    datatables: data,
                    id:id
    ...
    $('#table-aktivitas').on('click', '.btn-detail-media', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/check-mahasiswa/aktivitas/info',
                type: 'post',
                data: {
                    id: id
                },
