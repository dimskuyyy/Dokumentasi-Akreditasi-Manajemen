<div class="pull-left" style="display: flex;align-items:center; gap:1rem; margin-bottom: 1rem;">
    <a href="<?=base_url('wbpanel/check-mahasiswa')?>" type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-arrow-left"></i></a> <h4 style="display: inline-block;">Pengabdian Mahasiswa</h4>
</div><br><br>
<table id="table-pengabdian" class="table table-bordered table-hover table-striped" style="width:100%">
    <thead>
        <tr>
            ...
            <th data-priority='1'>Judul Pengabdian</th>
...
<script>
    ...
    var oTable = $('#table-pengabdian').dataTable({
        ...
        'ajax': function(data, callback, setting) {
            $.ajax({
                url: base_url + '/check-mahasiswa/pengabdian/list',
                type: 'post',
                data: {
                    datatables: data,
                    id:id
    ...
    $('#table-pengabdian').on('click', '.btn-detail-media', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/check-mahasiswa/pengabdian/info',
                type: 'post',
                data: {
                    id: id
                },
