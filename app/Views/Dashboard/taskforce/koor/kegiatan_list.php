<div class="pull-left" style="display: flex;align-items:center; gap:1rem; margin-bottom: 1rem;">
    <a href="<?=base_url('wbpanel/check-koor')?>" type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-arrow-left"></i></a> <h4 style="display: inline-block;">Kegiatan Koordinator Program Studi</h4>
</div><br><br>
<table id="table-kegiatan" class="table table-bordered table-hover table-striped" style="width:100%">
    <thead>
        <tr>
...
<script>
    ...
    var oTable = $('#table-kegiatan').dataTable({
        ...
        'ajax': function(data, callback, setting) {
            $.ajax({
                url: base_url + '/check-koor/kegiatan/list',
                type: 'post',
                data: {
                    datatables: data,
                    id:id
    ...
    $('#table-kegiatan').on('click', '.btn-detail-media', function(e) {
        e.preventDefault();
        let id = $(this).attr('data-id');
        if (id) {
            $.ajax({
                url: base_url + '/check-koor/kegiatan/info',
                type: 'post',
                data: {
                    id: id
                },
