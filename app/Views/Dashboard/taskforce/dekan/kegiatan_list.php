<div class="pull-left" style="display: flex;align-items:center; gap:1rem; margin-bottom: 1rem;">
    <a href="<?=base_url('wbpanel/check-dekan')?>" type="button" class="btn btn-sm btn-flat btn-primary btn-create"><i class="fa fa-arrow-left"></i></a> <h4 style="display: inline-block;">Kegiatan Fakultas</h4>
</div><br><br>
...
<script>
    ...
    var oTable = $('#table-kegiatan').dataTable({
        ...
        'ajax': function(data, callback, setting) {
            $.ajax({
                url: base_url + '/check-dekan/kegiatan/list',
                type: 'post',
                data: {
                    datatables: data,
                    id:id
    ...
    $('#table-kegiatan').on('click', '.btn-detail-media', function(e) {
        ...
        if (id) {
            $.ajax({
                url: base_url + '/check-dekan/kegiatan/info',
                type: 'post',
                data: {
                    id: id
                },
